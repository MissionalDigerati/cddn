...
describe Project do
  ...
  describe "methods" do
    ...
    it "should return a regular search if no params are provided including both open and closed projects returning projects with tags as well." do
      ...
      membership = FactoryGirl.create(:membership, user_id: user.id, project_id: project.id, role: "creator", status: "progress", creator_id: user.id)
      membership_not_open = FactoryGirl.create(:membership, user_id: user.id, project_id: project_not_open.id, role: "creator", status: "progress", creator_id: user.id)
      membership_with_tag = FactoryGirl.create(:membership, user_id: user.id, project_id: project_with_tag.id, role: "creator", status: "progress", creator_id: user.id)
    ...
    it "should search for projects with the searched tag, that are both open and closed for requests." do
      ...
      membership_not_open = FactoryGirl.create(:membership, user_id: user.id, project_id: project_not_open.id, role: "creator", status: "progress", creator_id: user.id)
      membership_with_tag = FactoryGirl.create(:membership, user_id: user.id, project_id: project_with_tag.id, role: "creator", status: "progress", creator_id: user.id)
      membership_without_tag = FactoryGirl.create(:membership, user_id: user.id, project_id: project_without_tag.id, role: "creator", status: "progress", creator_id: user.id)
    ...
    it "should filter projects based on if they are accepting requests or not" do  
      ...
      membership_not_open = FactoryGirl.create(:membership, user_id: user.id, project_id: project_not_open.id, role: "creator", status: "progress", creator_id: user.id)
      membership_open = FactoryGirl.create(:membership, user_id: user.id, project_id: project_open.id, role: "creator", status: "progress", creator_id: user.id)
    ...
    it "should filter projects by programming language tag as well as if they are open or not" do
      ...
      FactoryGirl.create(:membership, user_id: user.id, project_id: project_without_tag_accepting.id, role: "creator", status: "progress", creator_id: user.id)
      FactoryGirl.create(:membership, user_id: user.id, project_id: project_without_tag_not_accepting.id, role: "creator", status: "progress", creator_id: user.id)
      FactoryGirl.create(:membership, user_id: user.id, project_id: project_with_tag_not_accepting.id, role: "creator", status: "progress", creator_id: user.id)
      FactoryGirl.create(:membership, user_id: user.id, project_id: project_with_tag_accepting.id, role: "creator", status: "progress", creator_id: user.id)
